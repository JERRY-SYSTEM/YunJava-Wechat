...
地图.remove(qun+wxid);选择=0;return;}
...
String 描述="";
String 作者="";
if(!取(qun,"名字").equals("")){名字=取(qun,"名字");}
if(!取(qun,"描述").equals("")){描述=取(qun,"描述");}
if(!取(qun,"作者").equals("")){作者=取(qun,"作者");}
String result="智能体:"+名字+"\n来自于:"+作者+"\n描述语:\n"+描述;
if(取(qun,"名字").equals("")||取(qun,"描述").equals("")){result="当前默认体";}
sendm(qun,result);
}
...
if(text.contains("")){
text="带有特殊小表情，换个吧";
sendReply(data.msgId,qun,text);return;}
...
if(!取(qun,"对话").equals("")){记忆对话=取(qun,"对话");}
if(!取(qun,"智能体").equals("")){智能体=取(qun,"智能体");}
...
String message=matcher.group(1);
if(message.equals("参数错误")){
sendReply(data.msgId,qun,message);return;}
if(message.equals("success")){
JSONObject jsonObject = new JSONObject(jsonString);
String accessToken = jsonObject.getJSONObject("result").getString("access_token");
String refreshtoken = jsonObject.getJSONObject("result").getString("refresh_token");
...
}else{Toast("失败，请重新再试！");return;}
}else{sendReply(data.msgId,qun,message);return;}
...
String msg=json.getString("status_msg");
if(code==1111012161) {
sendReply(data.msgId,qun,"刷图过多，将不再生成图片");return;
}else if(code==1111012121) {
sendReply(data.msgId,qun,"内容涉及敏感信息");return;
} else if(code==1101010121||code==1118010010) {
sendReply(data.msgId,qun,msg);return;
}
}
return;
}else if(code==1111012121) {
